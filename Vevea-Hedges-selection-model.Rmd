...
$$
l\left(\boldsymbol\beta, \tau^2, \boldsymbol\omega\right) = \sum_{i=1}^k \ln w(T_i, \sigma_i^2) - \frac{1}{2} \sum_{i=1}^k \frac{\left(T_i - \mathbf{x}_i \boldsymbol\beta\right)^2}{\tau^2 + \sigma_i^2} - \frac{1}{2}\sum_{i=1}^k \ln\left(\tau^2 + \sigma_i^2\right) - \sum_{i=1}^k \ln A(\mathbf{x}_i \boldsymbol\beta, \tau^2, \boldsymbol\omega).
$$
...
To calculate the scores, we will need the derivatives of $A_i = A(\mathbf{x}_i \boldsymbol\beta, \tau^2, \boldsymbol\omega)$ with respect to $\boldsymbol\beta$ and $\tau^2$. Letting $b_{im} = -\sigma_i \Phi^{-1}(a_j) - \mathbf{x}_i \boldsymbol\beta$ and $\eta_i^2 = \tau^2 + \sigma_i^2$, we have

$$
\begin{aligned}
\frac{\partial A_i}{\partial \boldsymbol\beta} &= \frac{\mathbf{x}_i'}{\eta_i}\sum_{j=0}^q \omega_j \left[\phi\left(\frac{b_{i(j+1)}}{\eta_i}\right) - \phi\left(\frac{b_{ij}}{\eta_i}\right) \right] \\
\frac{\partial A_i}{\partial \tau^2} &= \sum_{j=0}^q \omega_j \left[ \frac{b_{i(j+1)}}{\eta_i^3} \phi\left(\frac{b_{i(j + 1)}}{\eta_i}\right) - \frac{b_{ij}}{\eta_i^3}\phi\left(\frac{b_{ij}}{\eta_i}\right) \right]
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial l}{\partial \boldsymbol\beta} &= \sum_{i=1}^k \frac{\mathbf{x}_i' \left(T_i - \mathbf{x}_i \boldsymbol\beta\right)}{\eta_i^2} - \sum_{i=1}^k \frac{\partial A_i / \partial \boldsymbol\beta}{A_i} \\
\frac{\partial l}{\partial \tau^2} &= \frac{1}{2}\sum_{i=1}^k \frac{\left(T_i - \mathbf{x}_i \boldsymbol\beta\right)^2}{\eta_i^4} - \frac{1}{2}\sum_{i=1}^k \frac{1}{\eta_i^2} - \sum_{i=1}^k \frac{\partial A_i/ \partial \tau^2}{A_i} \\
