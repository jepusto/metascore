---
title             : "On the connection between the Test of Excess Significance and weight-function models for selective publication"
...
This connection to the weight function model suggests that TES could be refined using score tests, a standard tool from mathematical statistics. Score tests [@rao1948LargeSampleTests] are asymptotically equivalent to likelihood ratio tests, but have the advantage that they do not require obtaining maximum likelihood estimates under the full (unrestricted) model [@boos1992GeneralizedScoreTests]. This feature is attractive in the present context because it allows one to circumvent potential convergence problems with the weight function model. Two forms of score tests are available---a parametric form and a robust form---which use different approaches to estimating the variance of the null score. 

The parametric score test [@rao1948LargeSampleTests] uses the Fisher information matrix to estimate the variance of the score function. Denote the full parameter vector of the weight function model as $\bs\theta = (\mu, \tau^2, \pi)$. Let $\bm{S}$ be the full score vector, so $\bm{S}(\bs\theta) = \partial l(\bs\theta) / \partial \bs\theta$. Let $\mathcal{I}$ denote the Fisher information matrix:
$$
